...
- JS
    ...
    app.get('/', async (req, res) => {
        ...
        if (id > 0) {
            ...
            // Deal pipelines (directions) — for the CATEGORY_ID field
            ...
            // Deal quotes — for the read-only QUOTE_ID field
        ...
        let sResult = ''
        let sResultCustom = ''
        // CONTACT_ID/CONTACT_IDS are deprecated — use crm.deal.contact.items.get
        ...
        for (const [key, field] of Object.entries(arResult.FIELDS)) {
            ...
            switch (field.type) {
                ...
                case 'crm_quote':
                    // only for QUOTE_ID — read-only, we show the quote names
...
- PHP
    ...
    <?php
        ...
        // Dictionaries are immediately converted into "code => label" maps for select.
        $arResult = [];
        ...
        {
            // CURRENCY in B24PhpSDK — Money\Currency object, the string code is retrieved via getCode()
        ...
        foreach ($arResult['FIELDS'] as $key => $arField)
        {
            ...
            switch ($arField['type'])
            {
                ...
                case 'crm_lead':
                    ...
                    break;
                case 'crm_company':
                    // The relation provides an identifier — we will fetch the name via crm.company.list
                    $arCompany = [];
                    if (!empty($value))
                    {
                    ...
                    break;
                case 'crm_contact':
                    // The relation provides an identifier — we will fetch the names via crm.contact.list
                    $arContact = [];
                    if (!empty($value))
                    {
...
- Python
    ...
    def select_field(params, options):
        """Builds <select> based on a \"value => label\" dictionary."""
        if not options:
            return ""
    ...
    def form_page():
        ...
        if item_id > 0:
            ...
            # Deal pipelines (directions) — for the CATEGORY_ID field.
            # The crm.dealcategory.list method does not have an SDK wrapper yet — call it directly.
        ...
        # The hidden ID field is added only for the edit form
...
- JS
    ...
    export async function handler(req, res) {
        ...
        // Convert values to the REST format according to the field type
        ...
        for (const [key, prop] of Object.entries(arFields)) {
            if (!(key in arForm)) {
                // Fields that are not in the form are cleared (except for read-only and files)
