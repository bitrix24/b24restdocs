...
- JS
    ...
    app.get('/', async (req, res) => {
        ...
        if (id > 0) {
            ...
            // Related company contacts (only their identifiers)
        ...
        for (const [key, field] of Object.entries(arResult.FIELDS)) {
            ...
            switch (field.type) {
                ...
                case 'crm_contact': {
                    ret = input({ ...params, TYPE: 'text' })
                    // Links only provide IDs — we fetch names via crm.contact.list
...
- PHP
    ...
    <?php
        ...
        {
            // CURRENCY in B24PhpSDK is a Money\Currency object; we get the string code via getCode()
        ...
        {//get item and standard enum field values if is update form
            ...
            // Related company contacts (links only provide identifiers)
        ...
        foreach ($arResult['FIELDS'] as $key => $arField)
        {
            ...
            switch ($arField['type'])
            {
                ...
                case 'crm_lead':
                    ...
                    break;
                case 'crm_contact':
                    // Links only provide identifiers — we will fetch names via crm.contact.list
                    $arContact = [];
...
- Python
    ...
    def form_page():
        ...
        # A hidden ID field is added only for the edit form
        hidden_id = ""
        if item.get("ID"):
...
- JS
    ...
    export async function handler(req, res) {
        ...
        // Convert values to REST format according to the field type
