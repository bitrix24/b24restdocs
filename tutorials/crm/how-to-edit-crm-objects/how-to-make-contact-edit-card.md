...
An example of automatic generation of a contact edit card containing all fields created in Bitrix24 on an application page.
...
- JS
    ...
    app.get('/', async (req, res) => {
        ...
        let sResult = ''
        let sResultCustom = ''
        // COMPANY_ID/COMPANY_IDS are deprecated — use crm.contact.company.items.get
...
- PHP
    ...
    <?php
        ...
        // Immediately convert dictionaries into "code => label" maps for select.
        $arResult = [];
        ...
        {
            // CURRENCY in B24PhpSDK is a Money\Currency object; retrieve the string code via getCode().
        ...
        {//get item and standard enum field values if is update form
            // Read the element as an array — the set of fields is dynamic (including UF_*).
...
- Python
    ...
    def form_page():
        ...
        # Insert a hidden ID field only when editing an existing contact
        hidden_id = ""
        if item.get("ID"):
...
- JS
    ...
    // req.body.form — an object containing form fields (Express with express.urlencoded()).
    export async function handler(req, res) {
        ...
        for (const [key, prop] of Object.entries(arFields)) {
            if (!(key in arForm)) {
                // Clear fields that are not present in the form (except read-only and files)
