...
An example of automatic generation of a lead edit card containing all fields created in Bitrix24 on an application page.

Some field types are not implemented in this example; for fields with unsupported types, the message *field not support* will be displayed.
...
- JS
    ...
    // Builds <input> (or a set for a multiple field)
    function input(p) {
    ...
    // Builds <select> from a "value => label" dictionary
    ...
    // Page with a form: we receive field descriptions and dictionary values, then render the form
    app.get('/', async (req, res) => {
        const id = parseInt(String(req.query.ID ?? '0'), 10) || 0

        // Field descriptions and standard list values (sequential calls)
        const arResult = {}
        ...
        if (id > 0) {
            // We get the element and values of related objects if it is an edit form
        ...
        // Render: builds the appropriate input/select based on each field type
        let sResult = ''
        let sResultCustom = ''
...
- PHP
    ...
    <?php
        ...
        // Field descriptions and dictionary values (sequential calls).
        // Dictionaries are immediately converted into "code => label" dictionaries for select.
        $arResult = [];
        ...
        {
            // CURRENCY in B24PhpSDK is a Money\Currency object; the string code is retrieved via getCode()
        ...
        {
            // The element is read as an array — the set of fields is dynamic (including UF_*)
            // Access to values is done via names from crm.lead.fields.
...
- Python
    ...
    def form_page():
        ...
        # A hidden field with ID is added only for the edit form
        hidden_id = ""
        if item.get("ID"):
...
Handler code that saves the form:
...
- JS
    ...
    // Handler: saves the form — updates an existing lead or creates a new one.
    // req.body.form — an object with form fields (Express with express.urlencoded()).
    export async function handler(req, res) {
        const arForm = { ...req.body.form }

        // Convert values to REST format based on field type
        ...
        for (const [key, prop] of Object.entries(arFields)) {
            if (!(key in arForm)) {
                // Fields not present in the form are cleared (except read-only and files)
...
- Python
    ...
    def save_form():
        # request.form contains keys in the form of form[FIELD]; collect into a nested dictionary
        ar_form = {}
