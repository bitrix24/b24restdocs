...
if ($_REQUEST['event'] == 'ONIMBOTMESSAGEADD') {
    ...
    $arReport['attach'][] = array("MESSAGE" => 'Once you sort out these tasks, just ask again [send=whats_burning]What\'s burning?[/send] and I will provide a new summary!');
    ...
    if ($_REQUEST['event'] == 'ONIMBOTJOINCHAT') {
        ...
        // send help message on how to use the chatbot. For private chat and for group chat need to send different instructions.
        $result = restCommand('imbot.message.add', array(
            'DIALOG_ID' => $_REQUEST['data']['PARAMS']['DIALOG_ID'],
            'MESSAGE'   => 'Hello! I am ReportBot, reporting everything as it is.',
            "ATTACH"    => array(
                array('MESSAGE' => '[send=что горит]What\'s burning?[/send]'),
            ),
        ), $_REQUEST["auth"]);
        ...
        if ($_REQUEST['event'] == 'ONIMBOTDELETE') {
            ...
            if ($_REQUEST['event'] == 'ONAPPINSTALL') {
                ...
                'PROPERTIES'            => array( // Personal data of the chatbot (required)
                    'NAME'              => 'ReportBot',
                    // Name of the bot (one of NAME or LAST_NAME is required)
                    'LAST_NAME'         => '',
                    // Last name of the bot (one of NAME or LAST_NAME is required)
                    'COLOR'             => 'AQUA',
...
function getAnswer($command = '', $user) {
    switch (strtolower($command)) {
        ...
        default:
            $arResult = array(
                'title' => 'I\'m stuck',
                'report'  => 'I can\'t figure out what you want to know. Or maybe I just can\'t...',
            );
    }
    return $arResult;
}
...
You can create a new chatbot application either as "for personal use" or in the account for publication to all _Bitrix24_ users via the _Market_.
...
- Configure the settings. At the top, specify the bot's name (ours is "ReportBot"). Then select the application type — **Server**:
...
## Launching the Chatbot for the On-Premise Account

For the on-premise version of *Bitrix24*, the process is slightly different:

1. Install and activate the PHP extension **cURL** on the server for the application to work correctly. In the BitrixVM virtual machine, this can be done through the menu: *7. Manage pool web servers > [1. Manage PHP extensions](https://dev.1c-bitrix.ru/learning/course/index.php?COURSE_ID=37&LESSON_ID=29362)*
2. Place the folder with the chatbot on your server or on your product installation
3. The application files must be accessible externally without authorization. Set the permissions on the folder with the chatbot — "read for unauthorized users". Access rights settings are performed in [folder properties](https://dev.1c-bitrix.ru/learning/course/index.php?COURSE_ID=35&LESSON_ID=2019)
4. On the account, go to the local application installation interface *Applications > Developer resources > Other > Local application*
5. Fill in the settings, similar to the instructions for the cloud account: 
