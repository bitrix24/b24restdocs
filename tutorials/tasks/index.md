...
- **Drive files.** To attach a file to a task, first upload the file to Drive using the [disk.folder.uploadFile](../../api-reference/disk/folder/disk-folder-upload-file.md) method. The method returns the Drive file `ID`. When creating a task, pass this identifier to the `UF_TASK_WEBDAV_FILES` field of the [tasks.task.add](../../api-reference/tasks/tasks-task-add.md) method with the `n` prefix, for example `n6687`. If the task has already been created, attach the file using the [tasks.task.files.attach](../../api-reference/tasks/tasks-task-files-attach.md) method: pass the file `ID` without the prefix in the `fileId` parameter, for example `6687`
