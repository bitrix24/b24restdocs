{% note alert "Code on the Bitrix24 domain only" %}

The `Messenger` object is available only to code that runs on the Bitrix24 page itself, on the same origin — page code and extensions. It is reached through `import { Messenger } from 'im.public'` or `BX.Runtime.loadExtension('im.public.iframe')`, both of which give direct access to the messenger runtime.
