{% note alert "The application runs in an iframe" %}
...
Calling these methods prints a deprecation notice in the portal console that recommends `Messenger.startVideoCall`, `Messenger.startPhoneCall`, or `Messenger.openChat`. Disregard it: those are top-window methods, available only to code running on the Bitrix24 page itself, and they are not reachable from an application. For that case, see [Passing Context to the Bot When Opening a Chat](/api-reference/chat-bots/chat-bots-v2/imbot.v2/bot-context.html).
