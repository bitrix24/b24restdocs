...
For images and videos, you can extend the block with `width` and `align` parameters, for example `[[image fileId=5001 width=30.98 align=left]]`.

The `width` parameter sets the width as a percentage of the editor container. The value can contain up to two decimal places. For a block without `align`, the allowed range is from `10` to `100`; for `align=left` or `align=right`, it is from `10` to `70`.

The `align` parameter sets alignment. Possible values: `left`, `right`. Center alignment is applied by default, so you do not need to pass `center`.
...
- cURL (Webhook)
    ...
    -d '{"id":77,"fields":{"title":"Chapter 1 (updated)","markdown":"# Chapter 1\n\nUpdated text\n\n[[image fileId=5001 width=30.98 align=left]]"},"overwrite":false}' \
