...
> [Comparison table of versions REST](#table)
...
- `/rest/api/` — indicates the version of REST. After `rest/`, specify `/api/` — this is the main difference in calling the new version from the old one. If `/api/` is not specified, Bitrix24 will execute the method from the old version of REST, if it exists, or return the error `Method not found` for methods available only in REST 3.0.
...
SDKs do not yet support the call address `/rest/api/`. Use direct HTTP requests, for example, via `curl`, `fetch`.

{% endnote %}

## OpenAPI Documentation {#openapi}

In REST 3.0, automatically generated documentation in the OpenAPI standard is available.
...
In the previous version of REST, different modules returned results in various formats. For example, the identifier of a created entity could be an embedded object `"result": { "id": 1823 }` or returned directly in the result `"result": 1823`. Different methods required writing custom logic for response handling.

The new version of REST returns the response to a request in a unified format that applies to all methods, regardless of the module.

### Structure of a Successful Response

If the method returns data, such as a list of found items or the identifier of a created item, they will be returned as an embedded object within `"result": {...}`.
...
If the method returns the result of an operation as true or false, for example, when deleting an item, it will return as an embedded object within `"result": {...}`.
...
2. ID is 1 or 2, **OR** ID is 3, 4, or 5.
...
|| `!=` | not equal | `["status", "!=", "CLOSED"]` → not closed ||
...
|| Number of Requests | More, all related objects are separate calls | Fewer, nested selections reduce the number of separate requests and lower the overall load on servers ||
