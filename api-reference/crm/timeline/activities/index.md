...
> 
> User documentation: [Activities in CRM](https://helpdesk.bitrix24.com/open/21648890/), [Activity Source](../../auxiliary/enum/outdated/crm-enum-activity-direction.md) 
...
To create, modify, or delete a system activity, use the methods from the [crm.activity.*](./activity-base/index.md) group. When creating a system activity, specify `TYPE_ID`, for example, for an e-mail activity `TYPE_ID` = `2`. To get values for other types of activities, use the method [crm.enum.activitytype](../../auxiliary/enum/outdated/crm-enum-activity-type.md).
...
Universal activities are a type of activity with extended parameters. In a universal activity card, you can synchronize the activity with the calendar, choose a meeting location with a customer, add employees, select a customer from a CRM entity, categorize activities by color, and choose a meeting room. Extended parameters are available to employees on the Bitrix24 side.

To create a universal activity, use the method [crm.activity.todo.add](./todo/crm-activity-todo-add.md). To change the expiration date of the activity, use the method [crm.activity.todo.updateDeadline](./todo/crm-activity-todo-update-deadline.md), and to change the description of the activity, use [crm.activity.todo.updateDescription](./todo/crm-activity-todo-update-description.md).
...
- Events
    ...
    #|
    || **Event** | **Triggered** ||
    || [onCrmActivityAdd](./events/on-crm-activity-add.md) | When creating an activity ||
    || [onCrmActivityUpdate](./events/on-crm-activity-update.md) | When updating an activity ||
    || [onCrmActivityDelete](./events/on-crm-activity-delete.md) | When deleting an activity ||
    |#
