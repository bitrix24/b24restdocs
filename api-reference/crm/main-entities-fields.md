...
> **Attention!** A more complete list of fields can be found on the pages of methods that return the description of the entity fields. Such methods are named **crm.entity_name.fields**.
...
- Deals
    ...
    || **ID**
    [`integer`][1] | Identifier of the deal. | Yes | No ||
    || **TITLE**^*^
    [`string`][1] | Title. | Yes | Yes ||
    || **TYPE_ID**
    [`crm_status`][1] | Type of the deal. Used only for linking to an external source. | Yes | Yes ||
    || **CATEGORY_ID**
    ...
    || **STAGE_ID**
    [`crm_status`][1] | Identifier of the stage.
...
(F - value for STAGE_SEMANTIC_ID) | Yes | Yes ||
    || **STAGE_SEMANTIC_ID**
    [`string`][1] | Name. Read-only, which in some sense generalizes the values of the deal identifier STAGE_ID. (See values above.) | Yes | No ||
    || **IS_NEW**
    [`char`][1] | Flag for a new deal (i.e., the deal is in the first stage). | Yes | No ||
    || **IS_RECURRING**
    [`char`][1] | Flag for a recurring deal template (if set to Y, it is not a deal but a template). | Yes | Yes ||
    || **IS_RETURN_CUSTOMER**
    [`char`][1] | Indicator of a repeat lead. | Yes | Yes ||
    || **IS_REPEATED_APPROACH**
    [`char`][1] | Repeat approach. | Yes | Yes ||
    || **PROBABILITY**
    [`integer`][1] | Probability. | Yes | Yes ||
    || **CURRENCY_ID**
    [`crm_currency`][1] | Identifier of the deal currency. | Yes | Yes ||
    || **OPPORTUNITY**
    ...
    || **COMPANY_ID**
    [`crm_company`][1] | Identifier of the linked company. | Yes | Yes ||
    || **CONTACT_ID**
    [`crm_contact`][1] | Identifier of the linked contact. Deprecated. Kept for compatibility. | Yes | Yes ||
    || **CONTACT_IDS**
    [`crm_contact`][1] | Identifier of the linked contact. Multiple.
When using [crm.deal.update](./deals/crm-deal-update.md) and [crm.deal.add](./deals/crm-deal-add.md), an array of contacts can be submitted. In the methods [crm.deal.list](./deals/crm-deal-list.md) and [crm.deal.get](./deals/crm-deal-get.md), this field is absent, and [crm.deal.contact.items.get](./deals/contacts/crm-deal-contact-items-get.md) should be used to get the list of contacts. To clear the field, use [crm.deal.contact.items.delete](./deals/contacts/crm-deal-contact-items-delete.md), to replace the value use [crm.deal.contact.items.set](./deals/contacts/crm-deal-contact-items-set.md). | Yes | Yes ||
    || **QUOTE_ID**
    [`crm_quote`][1] | Identifier of the quote. Read-only. Deprecated, use the method [crm.quote.list](./quote/crm-quote-list.md) with a filter by deal. | Yes | No ||
    || **BEGINDATE**
    [`date`][1] | Start date. | Yes | Yes ||
    || **CLOSEDATE**
    [`date`][1] | Completion date. | Yes | Yes ||
    || **OPENED**
    ...
    || **ASSIGNED_BY_ID**
    [`user`][1] | Linked to a user by ID. | Yes | Yes ||
    || **CREATED_BY_ID**
    [`user`][1] | Created by user. | Yes | No ||
    || **MODIFY_BY_ID**
    [`user`][1] | Identifier of the author of the last modification. | Yes | No ||
    || **MOVED_BY_ID**
    ...
    || **SOURCE_ID**
    [`string`][1] | Identifier of the source. Defines the source of the deal (callback, advertisement, e-mail, etc.). A list of possible identifiers can be retrieved using the REST method [crm.status.list](./status/crm-status-list.md) with the filter `filter[ENTITY_ID]=SOURCE` | Yes | Yes ||
    || **SOURCE_DESCRIPTION**
    [`string`][1] | Additional information about the source. Text field. | Yes | Yes ||
    || **ADDITIONAL_INFO**
    [`string`][1] | Additional information. | Yes | Yes ||
    || **LEAD_ID**
    [`crm_lead`][1] | Identifier of the linked lead. | Yes | No ||
    || **LOCATION_ID**
    [`location`][1] | Client's location. Service field, not recommended for use. | Yes | Yes ||
    || **ORIGINATOR_ID**
    [`string`][1] | Identifier of the data source. Used only for linking to an external source. | Yes | Yes ||
    || **ORIGIN_ID**
    [`string`][1] | Identifier of the item in the data source. Used only for linking to an external source. | Yes | Yes ||
    || **UTM_SOURCE**
    [`string`][1] | Advertising system. Yandex-Direct, Google-Adwords, and others. | Yes | Yes ||
    || **UTM_MEDIUM**
    ...
    || **PARENT_ID_xxx**
    [`crm_entity`][1] | Relationship fields. If there are SPAs related to contacts on the account, for each such SPA there is a field that stores the relationship between this SPA and the contact. The field itself stores the identifier of the item of that SPA. For example, the field `PARENT_ID_153` - relationship with the SPA `entityTypeId=153`, stores the identifier of the item of this SPA related to the current contact. | Yes | Yes ||
    || **LAST_ACTIVITY_BY**
...
- Leads
    ...
    || **HONORIFIC**
    [`crm_status`][1] | Form of address. Status from the directory. A list of possible identifiers can be obtained using the method [crm.status.list](./status/crm-status-list.md) with the filter `filter[ENTITY_ID]=HONORIFIC` | Yes | Yes ||
    || **NAME**
    ...
    || **SOURCE_ID**
    [`crm_status`][1] | Identifier of the source. Status from the directory. A list of possible identifiers can be obtained using the method [crm.status.list](./status/crm-status-list.md) with the filter `filter[ENTITY_ID]=SOURCE` | Yes | Yes ||
    || **SOURCE_DESCRIPTION**
    [`string`][1] | Description of the source. | Yes | Yes ||
    || **STATUS_ID**
    [`crm_status`][1] | Identifier of the lead stage. Status from the directory. A list of possible identifiers can be obtained using the method [crm.status.list](./status/crm-status-list.md) with the filter `filter[ENTITY_ID]=STATUS` | Yes | Yes ||
    || **STATUS_DESCRIPTION**
  ...
  - P (processing) – lead is being processed. | Yes | No ||
    ...
    || **CURRENCY_ID**
    [`crm_currency`][1] | Identifier of the currency. | Yes | Yes ||
    || **OPPORTUNITY**
    [`double`][1] | Estimated amount. | Yes | Yes ||
    || **IS_MANUAL_OPPORTUNITY**
    [`char`][1] | Indicator of manual calculation of the amount. Allowed values are Y or N. | Yes | Yes ||
    || **OPENED**
    [`char`][1] | Available to everyone. Allowed values are Y or N. | Yes | Yes ||
    || **COMMENTS**
    [`string`][1] | Comments. | Yes | Yes ||
    || **HAS_PHONE**
    [`char`][1] | Indicator of whether the phone field is filled. Allowed values are Y or N. | Yes | No ||
    || **HAS_EMAIL**
    [`char`][1] | Indicator of whether the email field is filled. Allowed values are Y or N. | Yes | No ||
    || **HAS_IMOL**
    [`char`][1] | Indicator of whether there is a linked open line. Allowed values are Y or N. | Yes | No ||
    || **ASSIGNED_BY_ID**
    [`user`][1] | Identifier of the user responsible for the lead. | Yes | Yes ||
    || **CREATED_BY_ID**
    [`user`][1] | Identifier of the user who created the lead. | Yes | No ||
    || **MODIFY_BY_ID**
    [`user`][1] | Identifier of the user who made the last modification. | Yes | No ||
    || **MOVED_BY_ID**
    [`user`][1] | Identifier of the user who moved the item to the current stage. | Yes | No ||
    || **DATE_CREATE**
    [`datetime`][1] | Creation date. | Yes | No ||
    || **DATE_MODIFY**
    [`datetime`][1] | Modification date. | Yes | No ||
    ...
    [`datetime`][1] | Date of moving the item to the current stage. | Yes | No ||
    || **COMPANY_ID**
    [`crm_company`][1] | Linking the lead to a company (Field Client->Company) | Yes | Yes ||
    || **CONTACT_ID**
    [`crm_contact`][1] | Linking the lead to a contact. Deprecated field, currently not used. Kept for backward compatibility. | Yes | Yes ||
    || **CONTACT_IDS**
    [`crm_contact`][1] | Identifier of the linked contact. Multiple. When using [crm.lead.update](./leads/crm-lead-update.md) and [crm.lead.add](./leads/crm-lead-add.md), an array of contacts can be submitted.  | Yes | Yes ||
    || **IS_RETURN_CUSTOMER**
    [`char`][1] | Indicator of a repeat lead. Allowed values are Y or N. | Yes | No ||
    || **DATE_CLOSED**
    ...
    || **PHONE**
    [`crm_multifield`][1] | Phone of the contact. Multiple. | Yes | Yes ||
    || **EMAIL**
    [`crm_multifield`][1] | E-mail address. Multiple. | Yes | Yes ||
    || **WEB**
    [`crm_multifield`][1] | URLs of the lead. Multiple. | Yes | Yes ||
    || **IM**
    [`crm_multifield`][1] | Messengers. Multiple. | Yes | Yes ||
    || **LINK**
    [`crm_multifield`][1] | Links. Multiple. Service field. | Yes | Yes ||
...
- Companies
  ...
  [crm.company.fields](./companies/crm-company-fields.md)
    ...
    || **TITLE**
    [`string`][1] | Title. Required | Yes | Yes ||
    || **COMPANY_TYPE**
    [`crm_status`][1] | Type of the company | Yes | Yes ||
    || **LOGO**
    [`file`][1] | Logo | Yes | Yes ||
    || **ADDRESS**
    ...
    || **REG_ADDRESS_2**
    [`string`][1] | Second line of the legal address. In some countries, it is customary to split the address into 2 parts. Deprecated, used for compatibility. | Yes | Yes ||
    || **REG_ADDRESS_CITY**
    ...
    || **INDUSTRY**
    [`crm_status`][1] | Industry | Yes | Yes ||
    || **EMPLOYEES**
    [`crm_status`][1] | Number of employees | Yes | Yes ||
    || **CURRENCY_ID**
    [`crm_currency`][1] | Currency | Yes | Yes ||
    || **REVENUE**
    ...
    [`string`][1] | Comments | Yes | Yes ||
    || **HAS_PHONE**
    [`char`][1] | Check for phone field completion | Yes | No ||
    || **HAS_EMAIL**
    [`char`][1] | Check for email field completion | Yes | No ||
    || **HAS_IMOL**
    [`char`][1] | Open line set | Yes | No ||
    || **IS_MY_COMPANY**
    ...
    || **LEAD_ID**
    [`crm_lead`][1] | Identifier of the lead associated with the company | Yes | No ||
    || **ORIGINATOR_ID**
    [`string`][1] | Identifier of the data source. Used only for linking to an external source. | Yes | Yes ||
    || **ORIGIN_ID**
    [`string`][1] | Identifier of the item in the data source. Used only for linking to an external source. | Yes | Yes ||
    || **ORIGIN_VERSION**
    [`string`][1] | Original version. Used to protect data from accidental overwriting by an external system. If the data was imported and not changed in the external system, such data can be edited in CRM without fear that the next export will lead to data overwriting | Yes | Yes ||
    || **UTM_SOURCE**
    [`string`][1] | Advertising system. Yandex-Direct, Google-Adwords, and others. | Yes | Yes ||
    || **UTM_MEDIUM**
    [`string`][1] | Type of traffic. CPC (ads), CPM (banners) | Yes | Yes ||
    ...
    [`string`][1] | Identifier of the user responsible for the last activity in this lead (for example, who created a new deal in the lead). | Yes | No ||
    || **PHONE**
    [`crm_multifield`][1] | Phone of the company. Multiple | Yes | Yes ||
    || **EMAIL**
    [`crm_multifield`][1] | E-mail address. Multiple | Yes | Yes ||
    || **WEB**
    [`crm_multifield`][1] | URLs of the company. Multiple | Yes | Yes ||
    || **IM**
    [`crm_multifield`][1] | Messengers. Multiple | Yes | Yes ||
    || **LINK**
    [`crm_multifield`][1] | Links. Multiple. Service field. | Yes | Yes ||
...
- Contacts
  ...
  [crm.contact.fields](./contacts/crm-contact-fields.md)
    ...
    ||**HONORIFIC**
    [`crm_status`][3] | Form of address.
    Values from the directory can be obtained using the method [crm.status.list](./status/crm-status-list.md) with the filter `ENTITY_ID=HONORIFIC` | Yes | Yes ||
    ||**NAME**
    ...
    [`date`][1] | Date of birth | Yes | Yes ||
    ||**TYPE_ID**
    [`crm_status`][3]| Type of contact.
    Values from the directory can be obtained using the method [crm.status.list](./status/crm-status-list.md) with the filter `ENTITY_ID=CONTACT_TYPE` | Yes | Yes ||
    ||**SOURCE_ID**
    [`crm_status`][3] | Source.
    Values from the directory can be obtained using the method [crm.status.list](./status/crm-status-list.md) with the filter `ENTITY_ID=SOURCE`| Yes | Yes ||
    ||**SOURCE_DESCRIPTION**
...
{% endnote %} | | ||
    ...
    ||**OPENED**
    [`char`][1] | Available to everyone. Can take values `Y` / `N`. Considered in the access permission work for roles with "All open" access level. | Yes | Yes ||
    ||**EXPORT**
    [`char`][1] | Participates in the export of contacts. Can take values `Y` / `N`.  | Yes | Yes ||
    ||**HAS_PHONE**
    [`char`][1] | Phone is set. Can take values `Y` / `N`. | Yes | No ||
    ||**HAS_EMAIL**
    [`char`][1] | E-mail is set. Can take values `Y` / `N`. | Yes | No ||
    ||**HAS_IMOL**
    [`char`][1] | Open line is set. Can take values `Y` / `N`. | Yes | No ||
    ||**ASSIGNED_BY_ID**
    [`user`][1] | Responsible | Yes | Yes ||
    ||**CREATED_BY_ID**
    [`user`][1] | Created by. | Yes | No ||
    ||**MODIFY_BY_ID**
    [`user`][1] | Modified by. | Yes | No ||
    ||**DATE_CREATE**
    [`datetime`][1] | Creation date. | Yes | No ||
    ||**DATE_MODIFY**
    [`datetime`][1] | Modification date. | Yes | No ||
    ||**COMPANY_ID**
    [`crm_company`][3] | Main company of the contact | Yes | Yes ||
    ||**COMPANY_IDS**
    [`crm_company`][3] | Linking the contact to companies. Multiple. In the methods [crm.contact.update](./contacts/crm-contact-update.md) and [crm.contact.add](./contacts/crm-contact-add.md), it is used to submit an array of companies. In the methods [crm.contact.list](./contacts/crm-contact-list.md) and [crm.contact.get](./contacts/crm-contact-get.md), this field is absent, and [crm.contact.company.items.get](./contacts/company/crm-contact-company-items-get.md) should be used to get the list of companies.  | Yes | Yes ||
    ||**LEAD_ID**
    [`crm_lead`][3] | Identifier of the lead associated with the contact. | Yes | No ||
|| | {% note tip "Fields for linking with external data sources" %}
...
{% endnote %} | | ||
    ...
    ||**ORIGIN_VERSION**
    [`string`][1] | Version of the contact data in the external system. Used to protect data from accidental overwriting by an external system. If the data was imported and not changed in the external system, such data can be edited in CRM without fear that the next export will lead to data overwriting. | Yes | Yes ||
    ||**FACE_ID**
    ...
    ||**UTM_TERM**
    [`string`][1] | Search condition of the campaign. For example, keywords for contextual advertising. | Yes | Yes ||
    ||**PARENT_ID_...** | Relationship fields. If there are SPAs related to contacts on the account, for each such SPA there is a field that stores the relationship between this SPA and the contact. The field itself stores the identifier of the item of that SPA. For example, the field `PARENT_ID_153` - relationship with the SPA `entityTypeId=153`, stores the identifier of the item of this SPA related to the current contact. ||
    ||**LAST_ACTIVITY_TIME**
    [`datetime`][1] | Date of the last activity in the timeline. | Yes | No ||
    ||**LAST_ACTIVITY_BY**
    [`user`][1] | Author of the last activity in the timeline. | Yes | No ||
    ||**PHONE**
    [`crm_multifield`][3] | Phones. Multiple. | Yes | Yes ||
    ||**EMAIL**
    [`crm_multifield`][3] | E-mail. Multiple. | Yes | Yes ||
    ||**WEB**
    [`crm_multifield`][3] | Websites. Multiple. | Yes | Yes ||
    ||**IM**
    [`crm_multifield`][3] | Messengers. Multiple. | Yes | Yes ||
    ||**LINK**
    [`crm_multifield`][3] | Links. Multiple. Service field. | Yes | Yes ||
    ||**UF_CRM_xxx**  | User-defined fields. For example, `UF_CRM_25534736`. Depending on the portal settings, contacts may have a set of user-defined fields of certain types. A user-defined field can be added to a contact using the method [crm.contact.userfield.add](./contacts/userfield/crm-contact-userfield-add.md).  ||
    |#


- Requisites

    ### General Requisites [crm.requisite.fields](./requisites/universal/crm-requisite-fields.md)

    #|
    || **Name**
    `type` | **Description** | **Read** | **Write** ||
    || **ID**
    [`integer`][1] | Identifier of the requisite, can be obtained using the method [crm.requisite.list](./requisites/universal/crm-requisite-list.md). Created automatically and unique within the portal. | Yes | No ||
    || **ENTITY_TYPE_ID**
    [`integer`][1] | Identifier of the parent entity type.

    Currently, this can only be:
    - `3` — contact
    - `4` — company

    Identifiers of all CRM entity types are returned by the method [crm.enum.ownertype][2]

    | Yes | Yes ||
    || **ENTITY_ID**
    [`integer`][1] | Identifier of the parent entity (contact or company).

    The identifier can be obtained using the method [crm.company.list](./companies/crm-company-list.md) for the company and the method [crm.contact.list](./contacts/crm-contact-list.md) for the contact | Yes | Yes ||
    || **PRESET_ID**
    ...
    || **ORIGINATOR_ID**
    [`string`][1] | Identifier of the external information base.
    ...
    || **RQ_IDENT_TYPE**
    [`crm_status`][1] | Method of identification | Yes | Yes ||
    || **RQ_IDENT_DOC**
    ...
    A user-defined field can be added to the requisites using the method [crm.requisite.userfield.add](./requisites/user-fields/crm-requisite-userfield-add.md) | Yes | Yes ||
    |#

    ### Bank Details [crm.requisite.bankdetail.fields](./requisites/bank-detail/crm-requisite-bank-detail-fields.md)
    ...
    Identifiers of object types are returned by the method [crm.enum.ownertype][2] | Yes | No ||
    || **ENTITY_ID**
    ...
    [`string`][1] | Symbolic code of the requisite | Yes | Yes ||
    || **XML_ID**
    [`string`][1] | External key. Used for exchange operations. Identifier of the object in the external information base.
    ...
    || **ACTIVE**
    [`char`][1] | Activity flag. Values `Y` or `N` are used.
    ...
    || **RQ_CODEB**
    [`string`][1] | Code Banque (for country FR) | Yes | Yes ||
    || **RQ_CODEG**
    [`string`][1] | Code Guichet (for country FR) | Yes | Yes ||
    || **RQ_RIB**
    [`string`][1] | Clé RIB (for country FR) | Yes | Yes ||
    || **RQ_MFO**
    ...
    || **RQ_ACC_TYPE**
    [`string`][1] | Tipo da conta (for country BR) | Yes | Yes ||
    || **RQ_AGENCY_NAME**
    [`string`][1] | Agência (for country BR) | Yes | Yes ||
    || **RQ_IIK**
    ...
    || **ORIGINATOR_ID**
    [`string`][1] | Identifier of the external information base. The purpose of the field may change by the end developer | Yes | Yes ||
    |#

    ### Templates of Requisites [crm.requisite.preset.fields](./requisites/presets/crm-requisite-preset-fields.md)
    ...
    Identifiers of CRM object types are returned by the method [crm.enum.ownertype][2] | Yes | Yes ||
    || **COUNTRY_ID**
    ...
    In CRM, reserved values of the form `#CRM_REQUISITE_PRESET_DEF_...` are used to identify templates that are used by default. These identifiers should not be used for your purposes, as this may lead to a violation of logic | Yes | Yes ||
    || **ACTIVE**
    [`char`][1] | Activity flag. Values `Y` or `N` are used. Determines the availability of the template in the selection list when adding requisites | Yes | Yes ||
    || **SORT**
    [`integer`][1] | Sorting | Yes | Yes ||
    |#

    ### Fields of Requisites Templates [crm.requisite.preset.field.fields](./requisites/presets/fields/crm-requisite-preset-field-fields.md)
    ...
    ### Addresses of Requisites [crm.address.fields](./requisites/addresses/crm-address-fields.md)
    ...
    Identifiers of object types can be obtained using the method [crm.enum.ownertype][2].
    {% note tip "" %}
...
{% endnote %} | Yes | Yes ||
    ...
    If the identifier of the location address module is specified when creating an address, a copy of the location address is created and linked to the created CRM address. If in this case no values are specified for the string address fields, they will be filled from the location address.

    If at least one string field was specified, then only the specified fields and their values will be saved in the CRM address, overwriting the corresponding values in the location address object. The same behavior will occur when updating the address | Yes | Yes ||
    || **ANCHOR_TYPE_ID**
    ...
    This field contains the identifier of the type of the parent object of the requisite (company or contact) if the address is linked to the requisite. If the address is linked to a lead, then this value will be the identifier of the lead | Yes | No ||
    || **ANCHOR_ID**
    [`integer`][1] | This field is for service use. The value is automatically filled when adding an address.

    This field contains the identifier of the parent object of the requisite (company or contact) if the address is linked to the requisite. If the address is linked to a lead, then this value will be the identifier of the lead | Yes | No ||
    |#
...
- Activities
    ...
    || **OWNER_TYPE_ID**
    [`crm.enum.ownertype`][1] | Type of owner, immutable | Yes | Yes ||
    || **TYPE_ID**
    [`crm_enum_activitytype`][1] | Type, immutable | Yes | Yes ||
    || **PROVIDER_ID**
    ...
    [`char`][1] | Completed | Yes | Yes ||
    || **STATUS**
    [`crm_enum_activitystatus`][1] | Status | Yes | Yes ||
    || **RESPONSIBLE_ID**
    [`user`][1] | Responsible | Yes | Yes ||
    || **PRIORITY**
    [`crm.enum.activitypriority`][1] | Importance | Yes | Yes ||
    || **NOTIFY_TYPE**
    [`crm.enum.activitynotifytype`][1] | Type of notifications | Yes | Yes ||
    || **NOTIFY_VALUE**
    [`integer`][1] | Notification parameter | Yes | Yes ||
    || **DESCRIPTION**
    [`string`][1] | Description | Yes | Yes ||
    || **DESCRIPTION_TYPE**
    [`crm.enum.contenttype`][1] | Type of description | Yes | Yes ||
    || **DIRECTION**
    [`crm.enum.activitydirection`][1] | Direction of the activity: incoming/outgoing. Relevant for calls and emails, not used for meetings. | Yes | Yes ||
    || **LOCATION**
    ...
    [`integer`][1] | | Yes | Yes ||
    || **RESULT_STREAM**
    [`integer`][1] | Statistics of reports | Yes | Yes ||
    || **RESULT_SOURCE_ID**
    [`string`][1] | | Yes | Yes ||
    || **PROVIDER_PARAMS**
    ...
    || **BINDINGS**
    [`crm_activity_binding`][1] | Bindings | Yes | No ||
    || **COMMUNICATIONS**
    [`crm_activity_communication`][1] | Communication channel. Multiple, required. | Yes | Yes ||
    || **FILES**
