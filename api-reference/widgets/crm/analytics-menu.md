...
|| `placement.bind` returns `ERROR_PLACEMENT_NOT_FOUND` | The code is specified incorrectly or the application has not been granted the `crm` scope. The placement is registered only with the `CRM_ANALYTICS_MENU` code ||
...
|| The item is not visible in the left menu of CRM Analytics | Expand the *Applications* item: the widget is displayed there, next to the *Bitrix24 Market* item ||
|#
