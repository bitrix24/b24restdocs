...
Task result methods are part of the [REST API 3.0](../../../rest-v3.md). Calls are made through the address `/rest/api/`.
...
**Task.** Each result is linked to a task via the `taskId` field. The task identifier can be obtained when [creating a task](../tasks-task-add.md), [retrieving a task](../tasks-task-get.md), or using the old method of [getting a list of tasks](../../tasks-task-list.md).
