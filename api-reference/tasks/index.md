...
Checklists help create a list of steps for completing the task. You can manage checklists using the group of methods [task.checklistitem.*](./checklist-item/index.md).

If you need to avoid manually filling in the same fields for recurring tasks, use [task templates](./template/index.md). A template allows you to save the title, description, participants, deadlines, project, checklist, and other parameters of the future task in advance.

Create links to tasks using the method [task.dependence.add](./task-dependence-add.md). Remove them using the method [task.dependence.delete](./task-dependence-delete.md).
...
In the new task detail form, discussions take place in the task chat. Starting from version `tasks 25.700.0`, comments have been moved to the chat, so use the task chat and messenger methods to work with messages. For more details, see the article [New Task Detail Form: Overview of Changes](./tasks-new.md).

The outcome of the work on a task can be recorded in a comment and fixed as a result. Manage task results using the group of methods [tasks.task.result.*](./result/index.md).
...
**Parent Task.** A task can have subtasks. In this case, it is considered a parent task. You can add a link to the parent task in the `PARENT_ID` parameter. You can obtain the task ID using the method for [creating a task](./tasks-task-add.md) or [getting a list of tasks](./tasks-task-list.md).

**Group or Project.** A task is linked by the group ID `GROUP_ID`. You can obtain the ID using the method for [creating a new group](../sonet-group/sonet-group-create.md) or the method for [getting a list of groups](../sonet-group/socialnetwork-api-workgroup-list.md).
...
**CRM.** You can link CRM objects to a task: contacts, companies, leads, deals, invoices, and SPAs. To link an object, specify its [identifier with a prefix](../crm/data-types.md#object_type) in the `UF_CRM_TASK` parameter. For example, `C_3` for a contact with `id = 3`. You can obtain the identifier using the method for [creating a new CRM item](../crm/universal/crm-item-add.md) or the method for [getting a list of items](../crm/universal/crm-item-list.md).
...
For more details on Scrum and its methods, see the article [Scrum: Overview of Methods](../sonet-group/scrum/index.md).
...
You can manage the stages of Kanban and "My Plan" using the group of methods [task.stages.*](./stages/index.md).

## Tasks in "Plan for the Day"

"Plan for the Day" is a to-do list of tasks and meetings that you have scheduled for the workday. The method [task.planner.getlist](./planner/index.md) retrieves the list of tasks from "Plan for the Day."
...
- [Tab in the task detail form](../widgets/task/view-tab.md) `TASK_VIEW_TAB`
- [Right panel of the task detail form](../widgets/task/view-sidebar.md) `TASK_VIEW_SIDEBAR`
- [Link at the top of the task detail form](../widgets/task/view-top-panel.md) `TASK_VIEW_TOP_PANEL`
...
- Methods
    ...
    || [tasks.task.pause](./tasks-task-pause.md) | Pauses the task and changes its status to "waiting for execution" ||
    ...
    || [tasks.task.disapprove](./tasks-task-disapprove.md) | Disapproves a task ||
...
|| [task.checklistitem.getmanifest](./checklist-item/task-checklist-item-get-manifest.md) | Retrieves a list of methods and their descriptions ||
|#
...
For the new task detail form, comment methods are not relevant. Task discussions take place in the task chat. For more details, see the article [New Task Detail Form: Overview of Changes](./tasks-new.md).
...
|| [task.elapseditem.update](./elapsed-item/task-elapsed-item-update.md) | Updates parameters of a time tracking record ||
...
|| [task.elapseditem.getmanifest](./elapsed-item/task-elapsed-item-get-manifest.md) | Retrieves a list of methods and their descriptions ||
|#
...
|| [task.item.userfield.getfields](./user-field/task-item-user-field-get-fields.md) | Retrieves all available fields of a custom field ||
|#
...
#| 
|| **Method** | **Description** ||
|| [task.stages.add](./stages/task-stages-add.md) | Adds stages to Kanban or "My Plan" ||
|| [task.stages.update](./stages/task-stages-update.md) | Updates stages of Kanban or "My Plan" ||
|| [task.stages.get](./stages/task-stages-get.md) | Retrieves stages of Kanban or "My Plan" ||
...
|| [task.stages.delete](./stages/task-stages-delete.md) | Deletes stages of Kanban or "My Plan" ||
|#
...
#| 
|| **Method** | **Description** ||
|| [task.planner.getList](./planner/task-planner-get-list.md) | Retrieves a list of tasks from "Plan for the Day" ||
|#
