...
A custom type is supported only when `IBLOCK_ID` or `IBLOCK_CODE` is provided. Without these parameters, the method returns lists only for types that are supported or registered in the Universal Lists module
